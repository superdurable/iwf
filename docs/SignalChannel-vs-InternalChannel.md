...
* Signal is like sending a message to MessageQueue without a response. Sometimes users do want to get some results as part of the write operation. So RPC is created. In order to let RPC to wake up some WorkflowStates running in background, we need some message channel.
