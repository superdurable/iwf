...
| WorkflowID| The business identifier to start a WorkflowExecution. It's like "Primary Key" of a database table. There cannot be more than one WorkflowExecutions running in-parallel with the same WorkflowID. It must be provided for any interactions after a workflow started. |
